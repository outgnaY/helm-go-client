...
// GetChartInfos added
func (c *Client) GetChartInfos() ([]*export.ChartInfo, error) {
	rr, err := c.cache.ListReferences()
	if err != nil {
		return nil, err
	}
	refsMap := map[string]map[string]interface{}{}
	for _, r := range rr {
		refsMap[r.Name] = map[string]interface{}{
			"name":    r.Chart.Metadata.Name,
			"version": r.Chart.Metadata.Version,
			"digest":  r.Manifest.Digest.Hex(),
			"size":    r.Size,
			"created": r.CreatedAt,
		}
	}
	...
	sort.Strings(keys)
	chartInfos := make([]*export.ChartInfo, len(refsMap))
	for i, key := range keys {
		ref := refsMap[key]
		chartInfo := &export.ChartInfo{
			Ref:       key,
			Name:      ref["name"].(string),
			Version:   ref["version"].(string),
			Digest:    ref["digest"].(string),
			Size:      ref["size"].(int64),
			CreatedAt: ref["created"].(time.Time),
		}
